export default function MapVisualization() {
  // 所有坐标均基于 viewBox="0 0 600 600" 的 SVG 坐标系
  // 点位坐标对应地图图片上的实际位置（百分比 × 600）
  const reservoirLabels = [
    { name: '刘庄水库',   px: 228, py: 108, textOffsetX: 10, textOffsetY: -8 },
    { name: '钟楼寺水库', px: 168, py: 192, textOffsetX: 10, textOffsetY: -8 },
    { name: '崖底水库',   px: 240, py: 240, textOffsetX: 10, textOffsetY: -8 },
    { name: '木头沟水库', px: 360, py: 432, textOffsetX: -80, textOffsetY: -14 },
  ];
  ...
  return (
    <div className="h-full w-full overflow-hidden relative bg-[#0a1628]">
      <style>{`
        ...
        .dot-ring  { animation: ring-expand 2s ease-out  infinite; }
        .dot-ring2 { animation: ring-expand 2s ease-out  infinite; animation-delay: -1s; }
      ...
      {/*
        单一 SVG 覆盖整个容器，viewBox 固定为 600×600。
        地图图片、动画圆圈、标注点、文字全部在同一坐标系内，
        无论容器如何缩放，各元素相对位置永远不变。
      */}
      <svg
        ...
        {reservoirLabels.map((item, i) => (
          <g key={i}>
            ...
            <line
              ...
              x2={item.px + item.textOffsetX * 0.8}
              y2={item.py + item.textOffsetY * 0.8}
              stroke="#00eaff" strokeWidth="0.8" opacity="0.5"
            />

            {/* 文字标注 */}
            <text
              x={item.px + item.textOffsetX}
              y={item.py + item.textOffsetY}
              fontSize="13"
              fontWeight="bold"
              fill="#00eaff"
