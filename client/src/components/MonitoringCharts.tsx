    ...
    <div className="h-full flex flex-col gap-3">
        ...
        <div className="bg-[#0f1d35] border border-accent/15 rounded-lg p-2" style={{ flex: '1 1 0', minHeight: '120px' }}>
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={rainfallData} margin={{ top: 8, right: 5, left: -15, bottom: 5 }}>
              ...
              <XAxis dataKey="time" stroke="#4a7a9b" style={{ fontSize: '8px' }} interval={5} tickLine={false} axisLine={{ stroke: '#1a3a52' }} />
              <YAxis stroke="#4a7a9b" style={{ fontSize: '8px' }} domain={[0, 100]} tickLine={false} axisLine={{ stroke: '#1a3a52' }}
      ...
      <div className="flex flex-col" style={{ flex: '1 1 0', minHeight: 0 }}>
        <div className="flex items-center gap-2 mb-1.5 flex-shrink-0">
          ...
          <span className="ml-auto text-[9px] text-red-400/70 font-mono">汛限 {waterLevelWarning}m</span>
        </div>
        <div className="bg-[#0f1d35] border border-accent/15 rounded-lg p-2" style={{ flex: '1 1 0', minHeight: '120px' }}>
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={waterLevelData} margin={{ top: 8, right: 8, left: -10, bottom: 5 }}>
              <defs>
              ...
              <CartesianGrid strokeDasharray="3 3" stroke="#1a3a52" opacity={0.3} />
              <XAxis dataKey="day" stroke="#4a7a9b" style={{ fontSize: '8px' }} tickLine={false} axisLine={{ stroke: '#1a3a52' }} />
              <YAxis stroke="#4a7a9b" style={{ fontSize: '8px' }} domain={[yMin, yMax]} tickLine={false} axisLine={{ stroke: '#1a3a52' }}
                label={{ value: 'm', angle: -90, position: 'insideLeft', offset: 8, fill: '#4a7a9b', fontSize: 9 }} />
              <Tooltip
                contentStyle={{ backgroundColor: '#0a1628', border: '1px solid rgba(0,212,255,0.3)', borderRadius: '6px', fontSize: '10px', boxShadow: '0 4px 12px rgba(0,0,0,0.5)' }}
                labelStyle={{ color: '#8ab4ff' }}
              ...
              <ReferenceLine y={waterLevelWarning} stroke="#ef4444" strokeWidth={1.5} strokeDasharray="4 3"
                label={{ value: `汛限 ${waterLevelWarning}m`, position: 'insideTopRight', fill: '#ef4444', fontSize: 9, fontWeight: 'bold' }} />
              {/* 正常蓄水位参考线 */}
              <ReferenceLine y={reservoirData.normalWaterLevel} stroke="#f59e0b" strokeWidth={1} strokeDasharray="3 4"
                label={{ value: `正常 ${reservoirData.normalWaterLevel}m`, position: 'insideBottomRight', fill: '#f59e0b', fontSize: 9 }} />
              <Area dataKey="level" stroke="#3b82f6" strokeWidth={1.5} fill="url(#waterGradient)" dot={{ r: 2, fill: '#3b82f6' }} isAnimationActive animationDuration={600} />
            </AreaChart>
          </ResponsiveContainer>
        </div>
      </div>
      ...
      <div className="flex flex-col" style={{ flex: '1 1 0', minHeight: 0 }}>
        <div className="flex items-center gap-2 mb-1.5 flex-shrink-0">
          ...
          <span className="ml-auto text-[9px] text-red-400/70 font-mono">警戒水头 {seepageWarningHead}m</span>
        </div>
        <div className="bg-[#0f1d35] border border-accent/15 rounded-lg p-2" style={{ flex: '1 1 0', minHeight: '140px' }}>
          <svg viewBox="0 0 320 180" className="w-full h-full" preserveAspectRatio="xMidYMid meet">
            ...
            <text x="59" y={warningY - 2} fontSize="7.5" fill="#ef4444" textAnchor="middle" fontWeight="bold">
